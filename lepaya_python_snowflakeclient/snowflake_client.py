...
class SnowflakeClient:
    ...
    def __enter__(self) -> SnowflakeClient:
        ...
        try:
            ...
            self.slack_client.add_error_block(error_msg, notify=True)
    ...
    def fetch_table_data(self, database: str, schema: str, table: str) -> pd.DataFrame:
        ...
        try:
            ...
            self.slack_client.add_error_block(error_message, notify=True)
    ...
    def run_query(self, query: str, table: str, schema: str, database: str) -> None:
        ...
        """
        LOGGER.info(
            f"Updating data in SnowflakeDB. Database: {database},"
            f"Schema: {schema}, Table: {table}",
        )
        try:
            if self.connection is None:
            ...
            log_and_update_slack(
                slack_client=self.slack_client,
                message=f"Successfully affected {cursor.rowcount} rows in"
                        f"in Table: {table}",
                temp=True,
            )
